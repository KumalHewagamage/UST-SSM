...
from models.UST_Seg import UST
...
class Config:
    # --- Experiment Settings ---
    experiment_name = "ust_4d_grounding_segmentation_v1"
    output_dir = "./outputs"
    seed = 42
    ...
    model_params = {
        ...
        'heads': 6,
        'mlp_dim': 320,
        'num_classes': 24, # 24 body part labels for segmentation
        'dropout': 0.0,
        'depth': 3, # 1
        'hos_branches_num': 3,
        'encoder_channel': 60
    ...
    # --- Loss Settings ---
    temperature = 0.07
    seg_loss_weight = 1.0  # Weight for segmentation loss (combined with text-grounding loss)
...
def compute_iou(pred_labels, gt_labels, num_classes):
    """
...
class CombinedLoss(nn.Module):
    """
    Combined loss for text-grounding and segmentation tasks.
    """
    def __init__(self, temperature: float = 0.07, seg_weight: float = 1.0):
        super().__init__()
        self.text_alignment_loss = TextActionAlignment(temperature=temperature)
        self.seg_weight = seg_weight
        self.seg_loss = nn.CrossEntropyLoss(ignore_index=-1)
    
    def forward(self, temporal_features, text_features, seg_logits=None, seg_labels=None):
        """
        Args:
            temporal_features: [B, D] - global scene features
            text_features: [B, D] - text features
            seg_logits: [B, L, N, num_classes] - segmentation logits
            seg_labels: [B, L, N] - ground truth labels
        
        Returns:
            total_loss, dict of individual losses
        """
        # Text-grounding loss
        text_loss = self.text_alignment_loss(temporal_features, text_features)
        ...
        if seg_logits is not None and seg_labels is not None:
            B, L, N, C = seg_logits.shape
            # Reshape: [B, L, N, C] -> [B*L*N, C]
            seg_logits_flat = seg_logits.reshape(-1, C)
            seg_labels_flat = seg_labels.reshape(-1)
            
            seg_loss = self.seg_loss(seg_logits_flat, seg_labels_flat)
...
class ValidationEvaluator:
    ...
    def evaluate(self, dataloader, logger=None, text_projection=None):
        ...
        with torch.no_grad():
            for i, (points, seg_labels, captions) in enumerate(tqdm(dataloader, desc="Validation")):
                ...
                # Model forward - returns both global features and segmentation logits
                global_features, seg_logits = self.model(points)
                
                if text_projection is not None:
                    global_features = text_projection(global_features)
                
                # Compute combined loss
                loss, loss_dict = self.criterion(global_features, text_features, seg_logits, seg_labels)
                total_loss += loss.item()
                total_text_loss += loss_dict.get('text_loss', 0.0)
                total_seg_loss += loss_dict.get('seg_loss', 0.0)
                
                # Compute segmentation IoU
                seg_pred = torch.argmax(seg_logits, dim=-1)  # [B, L, N]
                seg_pred_np = seg_pred.cpu().numpy().reshape(-1)
                seg_labels_np = seg_labels.cpu().numpy().reshape(-1)
                
                mean_iou, _ = compute_iou(seg_pred_np, seg_labels_np, num_classes=24)
                all_ious.append(mean_iou)
                
                # Store embeddings for retrieval metrics
                all_scene_embeddings.append(global_features.cpu())
                all_text_embeddings.append(text_features.cpu())
                
                # Batch-wise retrieval metrics (fixed batch size of 32)
                if global_features.shape[0] == 32:
                    sim = text_features @ global_features.t()
                    for k in self.k_values:
                        t2s_recall = self.compute_recall_at_k(sim, k, mode='text_to_scene')
                        s2t_recall = self.compute_recall_at_k(sim, k, mode='scene_to_text')
                        batch_recalls[f'recall@{k}_text_to_scene'].append(t2s_recall)
                        batch_recalls[f'recall@{k}_scene_to_text'].append(s2t_recall)
...
def train_one_epoch(model, text_encoder, loader, optimizer, criterion, device, epoch, logger, text_projection):
    ...
    for i, (points, seg_labels, captions) in enumerate(pbar):
        ...
        # Model Forward - returns global features and segmentation logits
        global_features, seg_logits = model(points)
        global_features = text_projection(global_features)

        # Combined Loss (text-grounding + segmentation)
        loss, loss_dict = criterion(global_features, text_features, seg_logits, seg_labels)
        ...
        running_text_loss += loss_dict.get('text_loss', 0.0)
        running_seg_loss += loss_dict.get('seg_loss', 0.0)
        
        # Update progress bar
        if i % Config.print_freq == 0:
            pbar.set_postfix({
                'loss': f"{loss.item():.4f}",
                'text': f"{loss_dict.get('text_loss', 0.0):.4f}",
                'seg': f"{loss_dict.get('seg_loss', 0.0):.4f}"
            })

    epoch_avg_loss = running_loss / len(loader)
    epoch_avg_text_loss = running_text_loss / len(loader)
    epoch_avg_seg_loss = running_seg_loss / len(loader)
    epoch_time = time.time() - start_time
    
    metrics = {
        'loss': epoch_avg_loss,
        'text_loss': epoch_avg_text_loss,
        'seg_loss': epoch_avg_seg_loss,
        'epoch_time': epoch_time,
        'num_batches': len(loader)
    }
    
    logger.info(f"Train Epoch [{epoch}] Avg Loss: {epoch_avg_loss:.4f} (Text: {epoch_avg_text_loss:.4f}, Seg: {epoch_avg_seg_loss:.4f}), Time: {epoch_time:.1f}s")
    return metrics
    ...
    for epoch in range(start_epoch, Config.epochs):
        ...
        if should_validate:
            ...
            tb_logger.log_validation_metrics(
                ...
                metrics={
                    'text_loss': val_metrics['text_loss'],
                    'seg_loss': val_metrics['seg_loss'],
                    'mean_iou': val_metrics['mean_iou'],
                    **{k: v for k, v in val_metrics.items() if k.startswith('recall@')}
                }
            )
            
            # Log segmentation metrics
            logger.info(f"Segmentation Metrics:")
            logger.info(f"  Mean IoU: {100*val_metrics['mean_iou']:.2f}%")
            logger.info(f"  Seg Loss: {val_metrics['seg_loss']:.4f}")
